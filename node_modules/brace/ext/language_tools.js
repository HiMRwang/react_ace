    ...
    (function() {
        ...
        this.openPopup = function(editor, prefix, keepPopupPosition) {
            ...
            if (!keepPopupPosition) {
                ...
                // getTextLeftOffset()  提示框左边的补偿宽度  5   px
                pos.left -= this.popup.getTextLeftOffset();


                // 代码区 1900 * 1200  top 8 left 8
                var rect = editor.container.getBoundingClientRect();
                //  pos.top + rect.top   pos.top +8 - 0  rect.top为8px向下偏移

                pos.top += rect.top - renderer.layerConfig.offset;
                //pos.left + rect.left   pos.top + 8 -0
                ...
                if (pat2.test(prefix)) {
                    ...
                    var str = prefix.substring(0, index + 1);
                    pos.left += str.length * 8;
                }
            ...
            if (data.completer && data.completer.insertMatch) {
                ...
                if (this.completions.filterText) {
                    ...
                    for (var i = 0, range; range = ranges[i]; i++) {
                        // 如果不是 以'.'结尾，补全的单词出现在输入前缀的列，以'.'结尾则出现在'.'后面
    ...
    (function() {
        this.setFilter = function(str) {
            ...
            function find(arr, str) {
                arr.forEach(function(item) {
                    if (item.value === str) {
                        ...
                        find(item.children, str)
                    }
                })
            }

            // 不以 '.'结尾但含有 '.'的输入前缀
            if (pat2.test(str) && str.indexOf('.') != -1) {
                var list = str.split('.');
                var str1 = list[0];
                // 倒数第二个'.'前面的单词
                var str2 = list[list.length - 2] || list[0];

                this.filterText = str.substring(str.lastIndexOf('.') + 1, str.length);
                matches.forEach(function(match) {
                    if (match.value === str1) {
                        if (match.value === str2 && (match.children)) {
                            dott = [...match.children];
                            //match.value!==倒数第二个'.'前面的单词
                        } else if (match.children) {
                            find(match.children, str2);
                            //没有子节点
                        } else if (!match.children) {
                            dott = _self.filterCompletions(matches, _self.filterText);
                        }
                        return dott
                    }
                })
                dott = _self.filterCompletions(dott, _self.filterText);
                dott = dott.sort(function(a, b) {
            ...
            if (pat1.test(str)) {
                ...
                var mid = str.split('.');
                var str1 = mid[0];
                var str2 = mid.length > 2 ? mid[mid.length - 2] : mid[0];
                var str3 = str.substr(0, str.length - 1)

                matches.forEach(function(match) {
                    if (match.value === str1) {
                        if ((match.value === str3) && (match.children)) {
                            dott = [...match.children];
                            return dott
                        } else if (match.children) {
                            find(match.children, str2);
        ...
        this.filterCompletions = function(items, needle) {
            ...
            loop:
                for (var i = 0, item; item = items[i]; i++) {
                    ...
                    // 如有处罚，就不是精准匹配, 当distance=0即首字母匹配时，penalty=0，为精准匹配
...
ace.define("ace/autocomplete/text_completer", ["require", "exports", "module", "ace/range"], function(acequire, exports, module) {
    ...
    function wordDistance(doc, pos) {
        ...
        //   createEmpty() 最终返回 { "__proto__": null };
...
module.exports = window.ace.acequire("ace/ext/language_tools")